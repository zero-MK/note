...
int *c = malloc(8);
```
在堆上分配了三个 chunk，看起来我们分配的是大小为 8 的的内存，其实在 x64 的系统下 ptmalloc 会给我们分配 16 字节的空间（对齐 16 字节），还有 chunk 的 prev_size 和 size 字段要占用 16 字节，所以 ptmalloc 会给我们分配的 **chunk 大小**为 32 字节（最小 chunk），这个 chunk 执行 free 操作后会放入 fastbin

![image-20201111002817222](image-20201111002817222.png)
...
```c
	...
	/* Check that the top of the bin is not the record we are going to add
	   (i.e., double free).  */
	if (__builtin_expect (old == p, 0)) // 如果 free 的 p chunk 等于 fastbin 的头节点，表明这个 chunk 刚刚已经 free 过了，直接提示 double free（fastbin 是 LIFO 的，并且使用的是头插法）
	  {
...
我们可以无限次轮流的分配到 b 和 a chunk，当然我们的目的不是这个，我们的目的是分配到 stack_var 的地址
...
使用是是这样的，因为一个 chunk 在使用（inuse）的状态下 fd 和 bk 是没有意义的，为了节约内存空间，我们的用户数据都是存在 fd 往后的内存中
