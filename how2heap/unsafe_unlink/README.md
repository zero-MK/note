...
正常情况下, smallbin 里面的 chunk 都是这样的,没有毛病
...
就是检查 p 的 fd 字段(存的是 地址)指向的 chunk (上图的 FD chunk)的 bk 必须指向 p

就是检查 p 的 bk 字段(存的是 地址)指向的 chunk (上图的 BK)的 fd 必须指向 p
...
我们构造了两个假的 chunk  （有点难看出来，其实就是在  0x555555558058 伪造了一个 chunk（bk），在 0x555555558060 伪造了一个 chunk（fd））
...
现在 chunk0_ptr 分别位于伪造的 chunk 的 fd 和 bk 的位置，直接可以绕过
